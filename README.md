...
| <a name="input_redis_access_policy_assignments"></a> [redis\_access\_policy\_assignments](#input\_redis\_access\_policy\_assignments) | Map of access policy name to a map of principals to assign.<br/>The inner key is a label used as the Terraform state key.<br/>Currently only 'Data Owner' is supported by Managed Redis.<br/><br/>Each inner entry:<br/>  - omit both fields to have the module create a new managed identity auto-named as '<redis-name>-<policy>-<label>-mi'<br/>  - set display\_name to override the auto-assembled identity name<br/>  - set object\_id to assign permissions to a pre-existing principal<br/><br/>Example:<br/>  redis\_access\_policy\_assignments = {<br/>    "Data Owner" = {<br/>      api-service  = {}                                                          # new MI, named: myapp-cache-sandbox-api-service-mi<br/>      jobs-worker  = { display\_name = "my-custom-mi" }                          # new MI, custom name<br/>      preexisting  = { object\_id = "00000000-0000-0000-0000-000000000000" }     # pre-existing, no MI created<br/>    }<br/>  } | <pre>map(map(object({<br/>    object_id    = optional(string)<br/>    display_name = optional(string)<br/>  })))</pre> | `{}` | no |
